...
public class MecanumTeleop extends OpMode {
	...
	private int s_grabState = 2; //start in rest
	private double[] s_rightPositions = {0.1, 0.5, 0.9}; //grab to rest position
	private double[] s_leftPositions = {0.9, 0.5, 0.01};
	...
	public void loop() {
		...
		*/
		...
		if (rightTrigger && !previousRightTrigger) {
			grabState = !grabState;
			robot.rightArm.setPosition(grabState ? robot.RIGHT_ARM_GRAB_POSITION : robot.RIGHT_ARM_REST_POSITION);
			robot.leftArm.setPosition(grabState ? robot.LEFT_ARM_GRAB_POSITION : robot.LEFT_ARM_REST_POSITION);
			robot.topArm.setPosition(grabState ? robot.TOP_ARM_GRAB_POSITION : robot.TOP_ARM_REST_POSITION);
		} else if (leftTrigger && !previousLeftTrigger) { //incrementally let go
			robot.rightArm.setPosition(robot.rightArm.getPosition() + 0.05);
			robot.leftArm.setPosition(robot.leftArm.getPosition() - 0.05);
			robot.topArm.setPosition(robot.topArm.getPosition() - 0.05);
		}
		...
		*/
		...
		if (rightBumper && !previousRightBumper) {
			s_grabState++;
			if (s_grabState >= s_rightPositions.length) {
				s_grabState = 0;
			}
			s_targetRight = s_rightPositions[s_grabState];
			s_targetLeft = s_leftPositions[s_grabState];
